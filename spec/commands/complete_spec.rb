require 'spec_helper'
require_relative '../../src/commands/complete'
require_relative '../../src/commands/add'
require 'tempfile'

RSpec.describe TodoCli::Commands::Complete do
  ...
  describe '#execute' do
    context 'when todo exists' do
      ...
      it 'completes the todo successfully' do
        command = described_class.new('1')

        expect { command.execute }.to output(/✓ Completed todo #1: Buy groceries/).to_stdout
      end
      ...
      it 'returns a successful result object' do
        ...
        expect(result.message).to eq('Todo completed')
      end

      it 'prevents completing already completed todo' do
        # Complete the todo first
        capture_stdout { described_class.new('1').execute }

        # Try to complete again
        command = described_class.new('1')

        expect { command.execute }.to output(/Todo #1 is already completed/).to_stdout
      end
    end
