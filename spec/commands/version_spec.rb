...
RSpec.describe TodoCli::Commands::Version do
  describe '#execute' do
    context 'with default output' do
      it 'displays formatted version information' do
        command = described_class.new

        expect { command.execute }.to output(/TodoCli/).to_stdout
    ...
    context 'with JSON output' do
      it 'outputs valid JSON format' do
        ...
        expect(json_data['name']).to eq('TodoCli')
  ...
  describe 'constants' do
    it 'has correct version number' do
      expect(TodoCli::Commands::Version::VERSION).to eq('1.0.0')
    end

    it 'has build date' do
      expect(TodoCli::Commands::Version::BUILD_DATE).to eq('2025-01-15')
    end
  end
